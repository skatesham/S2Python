...
def avaliar(expressao):
    ...
    Execução tempo espaço O(n)
    :param expressao: string com expressão aritmética
    :return: valor númerico com resultado
    """

    def calculator(ultimo, fila1, item):
        valor = ultimo
        if fila1.vazia():
            return valor
        elif item.topo() in '+':  # Interpletando Caractere
            valor = fila1.desempilhar() + valor
        elif item.topo() in '-':  # Interpletando Caractere
            valor = fila1.desempilhar() - valor
        if item.topo() in '*':  # Interpletando Caractere
            valor = fila1.desempilhar() * valor
        elif item.topo() in '/':  # Interpletando Caractere
            valor = fila1.desempilhar() / valor
        item.desempilhar()
        return calculator(valor, fila1, item)


    def Repercursar(filarecursiva, primeiro):  # cria 2 pilhas para contas
        pilhaprox = Pilha()
        pilhafunc = Pilha()
        conta = 0
        if num(primeiro):
            pilhaprox.empilhar(primeiro)
            if filarecursiva.vazia():
                return pilhaprox.desempilhar()
        while True:
            if num(filarecursiva.primeiro()):
                pilhaprox.empilhar(filarecursiva.desenfileirar())
            elif filarecursiva.primeiro() in ('+-*/'):
                desempilhamento = filarecursiva.desenfileirar()
                if desempilhamento in ('*/') and num(filarecursiva.primeiro()):
                    if desempilhamento in '*':
                        conta = pilhaprox.desempilhar() * filarecursiva.desenfileirar()
                        pilhaprox.empilhar(conta)
                    if desempilhamento in '/':
                        conta = pilhaprox.desempilhar() / filarecursiva.desenfileirar()
                        pilhaprox.empilhar(conta)
                else:
                    pilhafunc.empilhar(desempilhamento)
            else:
                filarecursiva.desenfileirar()
            if filarecursiva.vazia():
                break
        save = pilhaprox.desempilhar()
        conta += save
        if pilhaprox.vazia():
            return conta
        return calculator(save, pilhaprox, pilhafunc)

    valor = ''
    fila = analise_sintatica(analise_lexica(expressao))
    if fila.vazia():
        raise ErroSintatico  # corrigido erro Vazio
    if num(fila.primeiro()):
        valor = fila.desenfileirar()
    if fila.vazia():
        return valor
    return Repercursar(fila, valor)
